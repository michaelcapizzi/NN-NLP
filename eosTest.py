    ...
    if i == 0:
        ...
        for line in f:
            ...
            if len(line.rstrip().split(" ")) < max_sentence_length:
                ...
                if np.any(lemmaVector == np.zeros(w2v_dimension)):
...
finalPrecision = float(finalTP) / float(finalTP + finalFP)
finalRecall = float(finalTP) / float(finalTP + finalFN)
finalF1 = float(2 * finalPrecision * finalRecall) / float(finalPrecision + finalRecall)
